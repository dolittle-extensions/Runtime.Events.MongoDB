...
namespace Dolittle.Runtime.Events.Specs.MongoDB
{
    ...
    public class when_persisting_event_context : Store.Specs.given.an_event_store
    {
        ...
        {
            return new UncommittedEventStream(uncommitted.Id,uncommitted.CorrelationId,uncommitted.Source,
        ...
        {
            return new EventMetadata(metadata.Id, metadata.VersionedEventSource, metadata.CorrelationId,
                                        metadata.Artifact, metadata.Occurred, build_new_original_context(metadata.OriginalContext));
        }

        static OriginalContext build_new_original_context(OriginalContext context)
        {
            return new OriginalContext(context.Application, context.BoundedContext, context.Tenant,
                context.Environment, build_claims(), context.CommitInOrigin);
        }
